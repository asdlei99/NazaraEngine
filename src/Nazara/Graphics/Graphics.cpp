	...
	*/
	...
	Graphics::~Graphics()
	{
		...
		if (defaultAtlas && defaultAtlas->GetStorage() == DataStorage::Hardware)
		{
			...
			// The default font can make live one hardware atlas after the free of a module (which could be problematic)
			// So, if the default font use a hardware atlas, we stole it.
			// I don't like this solution, but I don't have any better
			if (defaultAtlas.use_count() > 1)
			{
				// Still at least one font use the atlas
