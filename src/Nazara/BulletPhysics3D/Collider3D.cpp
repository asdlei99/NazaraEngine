...
namespace Nz
{
	...
	Boxf Collider3D::ComputeAABB(const Matrix4f& offsetMatrix, const Vector3f& scale) const
	{
		...
		transform.setBasis(btMatrix3x3(
			offsetMatrix.m11, offsetMatrix.m12, offsetMatrix.m13,
			offsetMatrix.m21, offsetMatrix.m22, offsetMatrix.m23,
			offsetMatrix.m31, offsetMatrix.m32, offsetMatrix.m33
		));

		transform.setOrigin(btVector3(offsetMatrix.m41, offsetMatrix.m42, offsetMatrix.m43));

		btVector3 min, max;
		GetShape()->getAabb(transform, min, max);

		return Boxf(scale * Vector3f(min.x(), min.y(), min.z()), scale * Vector3f(max.x(), max.y(), max.z()));
	}
	...
	void Collider3D::ComputeInertia(float mass, Vector3f* inertia) const
	{
		...
		*inertia = Vector3f(inertiaVec.x(), inertiaVec.y(), inertiaVec.z());
	}
	...
	m_lengths(lengths)
	{
		static_assert(sizeof(m_storage) >= sizeof(btBoxShape));

		btBoxShape* shape = reinterpret_cast<btBoxShape*>(&m_storage);
		PlacementNew(shape, btVector3{ lengths.x, lengths.y, lengths.z });
	}

	BoxCollider3D::~BoxCollider3D()
	{
		btBoxShape* shape = reinterpret_cast<btBoxShape*>(&m_storage);
		PlacementDestroy(shape);
	}
	...
	{
		return reinterpret_cast<btBoxShape*>(&m_storage);
	}
